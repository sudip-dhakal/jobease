...
const DropdownSkills = () => {
  const [selectedSkills, setSelectedSkills] = useState(["Node", "React", "Js"]);
  return (
    <>
      <div className="flex fixed inset-0 items-center justify-center backdrop-blur-sm m-auto ">
        <div className="text-white  bg-opacity-50 z-50 h-[60%] w-[50%] rounded-2xl  border-amber-600 border border-dotted bg-gray-800 p-4">
          <div className="flex justify-between items-center pb-4 border-b border-solid border-gray-300">
            ...
            <div className="flex justify-between gap-x-10 items-center">
              ...
              <button className="px-4 py-2 cursor-pointer hover:bg-red-600 duration-300 bg-red-700 border-gray-200 rounded-lg">
                Close
              </button>
            </div>
          </div>

          <div className="w-[80%] bg-[#d9d9d9] p-2 mx-auto rounded-sm min-h-[10%] mt-5 flex flex-wrap gap-x-5">
          ...
          <div className="w-full gap-3 mt-3 border-x border-solid border-gray-600 h-[50%] flex text-[#d9d9d9] flex-wrap ">
            {skills.map((skill) => {
              return (
                <div className=" hover:bg-amber-800 px-4 py-2 rounded-md cursor-pointer border items-center ">
                  {skill}
                </div>
              );
            })}
